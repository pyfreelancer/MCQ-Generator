...
from typing_extensions import Annotated # Required for Pydantic v2 custom types
from pydantic.functional_validators import BeforeValidator # Required for Pydantic v2 custom types
# Removed: from pydantic_core import CoreSchema
# Removed: from pydantic.json_schema import GetJsonSchemaHandler

# New (Pydantic v2 style for ObjectId handling - more robust)
# Define a custom validator function for ObjectId that returns a string
def validate_object_id_str(v: Any) -> str:
    """
    Custom validator for PyObjectId.
    Handles conversion from ObjectId to str, and validates str inputs as ObjectId.
    Always returns a string representation of the ObjectId.
    """
    if isinstance(v, ObjectId):
        return str(v)
    if isinstance(v, str) and ObjectId.is_valid(v):
        return v # If it's a valid string ObjectId, keep it as string
    raise ValueError('Invalid ObjectId string or ObjectId object')

# Use Annotated for PyObjectId, making it a string type that's validated as an ObjectId
PyObjectId = Annotated[str, BeforeValidator(validate_object_id_str)]
...
class MCQItem(BaseModel):
    """
    Represents a single Multiple Choice Question as parsed directly from LLM output.
    Includes the question text, a list of options, and the correct answer string.
    """
    question: str
    options: List[str]
    correct_answer: str # This is the text of the correct answer, not its index
...
class QuestionInDB(QuestionBase):
    ...
    model_config = {
        ...
        "arbitrary_types_allowed": True, # Required for PyObjectId (as it's a custom Annotated type)
        "json_schema_extra": {
            "examples": [
                {
            ...
            "examples": [
                {
                    ...
                    "quiz_date": "2023-10-26T11:00:00:00.000Z",
                    "total_questions": 10,
                    "correct_answers": 7,
                    "score": 70.0
                }
